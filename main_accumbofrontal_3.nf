...
process Subcortex_segmentation {
    ...
    output:
    tuple val(sid), file("${sub}_${ses}_all_fast_firstseg.nii.gz"), emit: sub_parcels
...
process Tractography_registration {
    input:
    tuple val(sub), val(ses), path(T1nativepro_brain), path(affine), path(warp), path(tracto_pft)

    output:
    tuple val(sub), val(ses), file("${bname}_t1.trk")
...
process Fs_ciftify {
    input:
    tuple val(sid), path(seg_first), path(lh_surf_shape), path(rh_surf_shape), path(lh_surf), path(rh_surf), path(lh_surf_sphere), path(rh_surf_sphere), path(lh_surf_sphere_fslr), path(rh_surf_sphere_fslr), path(labels)

    output:
    tuple val(sid), file("${sid}_resampled_lh.surf.gii"), file("${sid}_resampled_rh.surf.gii"), emit: res_surfaces
    tuple val(sid), file("${sid}_resampled_lh.dscalar.nii"), file("${sid}_resampled_rh.dscalar.nii"), emit: res_metric
    tuple val(sid), file("${sid}.dscalar.nii"), emit: dscalar
    tuple val(sid), file("${sid}_volume_label.nii.gz"), emit: volume
    
    

    script:
    """   
    wb_command -surface-resample $lh_surf $lh_surf_sphere $lh_surf_sphere_fslr BARYCENTRIC ${sid}_resampled_lh.surf.gii
    wb_command -surface-resample $rh_surf $rh_surf_sphere $rh_surf_sphere_fslr BARYCENTRIC ${sid}_resampled_rh.surf.gii
    wb_command -metric-resample $lh_surf_shape $lh_surf_sphere $lh_surf_sphere_fslr BARYCENTRIC ${sid}_resampled_lh.dscalar.nii
    wb_command -metric-resample $rh_surf_shape $rh_surf_sphere $rh_surf_sphere_fslr BARYCENTRIC ${sid}_resampled_rh.dscalar.nii
    wb_command -volume-label-import  ${seg_first} ${labels} ${sid}_volume_label.nii.gz
    wb_command -cifti-create-dense-scalar ${sid}.dscalar.nii -volume ${seg_first} ${sid}_volume_label.nii.gz -left-metric ${sid}_resampled_lh.dscalar.nii -right-metric ${sid}_resampled_rh.dscalar.nii
    """
}
...
process Save_cifti {
    ...
    output:
    tuple val(sid), file("${sid}_nac.dscalar.nii"), emit: dscalar
    

    script:
    """   
    python /home/pabaua/dev_tpil/tpil_dmri/surface/CSS_2.py $tracto_pft $lh_surf $rh_surf $dscalar ${sid}_nac.dscalar.nii
    """
}
...
workflow {
    ...
    input_fmriprep = file(params.input_fp)
    template = Channel.fromPath("$params.template")

    fs_brain = Channel.fromPath("$input_freesurfer/**/brain.mgz").map{[it.parent.parent.name.split(/_/)[0], it.parent.parent.name.split(/_/)[1], it.parent.parent.parent]}
    ...
    // hcp files
    // lh_surf_shape = Channel.fromPath("$input_fmriprep/*/anat/*_hemi-L_thickness.shape.gii").map{[it.parent.parent.name, it]}
    // rh_surf_shape = Channel.fromPath("$input_fmriprep/*/anat/*_hemi-R_thickness.shape.gii").map{[it.parent.parent.name, it]}
    // lh_surf = Channel.fromPath('/home/pabaua/dev_tpil/results/results_fmriprep/24-01-16_fmriprep/sub-007/anat/sub-007_hemi-L_white.surf.gii')
    // rh_surf = Channel.fromPath('/home/pabaua/dev_tpil/results/results_fmriprep/24-01-16_fmriprep/sub-007/anat/sub-007_hemi-R_white.surf.gii')
    // lh_surf_sphere = Channel.fromPath('/home/pabaua/dev_tpil/results/results_fmriprep/24-01-16_fmriprep/sub-007/anat/sub-007_hemi-L_sphere.surf.gii')
    // rh_surf_sphere = Channel.fromPath('/home/pabaua/dev_tpil/results/results_fmriprep/24-01-16_fmriprep/sub-007/anat/sub-007_hemi-R_sphere.surf.gii')
    // lh_surf_sphere_fslr = Channel.fromPath('/home/pabaua/neuromaps-data/atlases/fsLR/tpl-fsLR_den-32k_hemi-L_sphere.surf.gii')
    // rh_surf_sphere_fslr = Channel.fromPath('/home/pabaua/neuromaps-data/atlases/fsLR/tpl-fsLR_den-32k_hemi-R_sphere.surf.gii')
    ...
    // t1_nativepro_brain.combine(t1_to_diff_affine, by:[0,1]).combine(t1_to_diff_warp, by:[0,1]).combine(tracto_diff_pft, by:[0,1]).set{data_tracto_reg}
    ...
    // Subcortex_segmentation.out.sub_parcels.combine(lh_surf_shape).combine(rh_surf_shape).combine(lh_surf).combine(rh_surf).combine(lh_surf_sphere).combine(rh_surf_sphere).combine(lh_surf_sphere_fslr).combine(rh_surf_sphere_fslr).combine(labels).set{data_ciftify}
    // Fs_ciftify(data_ciftify)

    // Tractography_registration.out.combine(Fs_ciftify.out.res_surfaces, by:0).combine(Fs_ciftify.out.dscalar, by:0).set{data_save_cifti}
